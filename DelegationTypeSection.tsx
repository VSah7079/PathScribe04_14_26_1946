...
const Form: React.FC<FormProps> = ({ initial, existingIds, onSave, onCancel }) => {
  ...
  return (
    <div style={{background:'rgba(138,180,248,0.04)',border:'1px solid rgba(138,180,248,0.2)',borderRadius:10,padding:20,display:'flex',flexDirection:'column',gap:14}}>
      <div style={{fontSize:13,fontWeight:700,color:'#8AB4F8'}}>{isNew?'+ New Delegation Type':`Editing â€” ${initial!.label}`}</div>
      ...
      <div>
        ...
        <textarea value={form.description} rows={2} placeholder="Shown to staff during delegationâ€¦" onChange={e=>set('description',e.target.value)} style={{...inp(!!errors.description),resize:'vertical'}}/>
      ...
      <div>
        ...
        <input value={form.cptHint??''} placeholder="e.g. 88321â€“88325" onChange={e=>set('cptHint',e.target.value||undefined)} style={inp()}/>
      </div>
  ...
  <div style={{display:'flex',alignItems:'center',gap:12,padding:'11px 14px',borderRadius:8,background:dt.active?'rgba(255,255,255,0.03)':'rgba(255,255,255,0.01)',border:'1px solid rgba(255,255,255,0.07)',opacity:dt.active?1:0.5,transition:'opacity 0.2s'}}>
    ...
    <div style={{flex:1,minWidth:0}}>
      <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
        ...
        {dt.isSystem&&<span title="System type â€” cannot be deleted" style={{fontSize:10,color:'#475569',fontWeight:600}}>ðŸ”’ system</span>}
    ...
    {!dt.isSystem&&(
      deleteConfirm===dt.id?(
        ...
        <button onClick={()=>onDeleteRequest(dt.id)} disabled={editingAny} style={{padding:'5px 10px',borderRadius:6,fontSize:11,cursor:editingAny?'not-allowed':'pointer',background:'transparent',border:'1px solid transparent',color:'#475569',flexShrink:0,opacity:editingAny?0.4:1}}>âœ•</button>
      )
    )}
  </div>
);
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:24}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
        ...
        {!editingAny&&(
          <button className="ps-section-add-btn" onClick={() => setEditing('new')}>+ Add Type</button>
        )}
      </div>
      ...
      {loading ? <div style={{fontSize:13,color:'#475569'}}>Loadingâ€¦</div> : (
        <>
          <div>
          ...
          <div>
            ...
            {customTypes.length===0?(
              <div style={{fontSize:13,color:'#334155',padding:'12px 0'}}>No custom types yet â€” create one above.</div>
            ):(
              <div style={{display:'flex',flexDirection:'column',gap:6}}>
                {customTypes.map(dt=>{
