...
const SEED_CROSSWALK: SpecimenCodeCrosswalkEntry[] = [
  {
    ...
    specimenCategoryId: 'cat-surgical-tissue', createdAt: '2026-05-10', createdBy: 'admin',
  },
  {
    id: 'xwalk-002', clientId: 'c-royal-manchester', externalCode: 'CYTO-FL',
    specimenCategoryId: 'cat-fluid-cytology', createdAt: '2026-05-12', createdBy: 'admin',
  },
];
...
//   ORD-001 — Fenwick General, known client code + crosswalked specimen
//             code → resolves cleanly, no auto-creation at all.
//   ORD-002 — client code 'NEWCLINIC01' doesn't match any existing
//             Client.code → demonstrates clientWasAutoCreated.
...
const SEED_ORDERS: IncomingOrder[] = [
  ...
  },
  {
    ...
    externalClientCode: 'NEWCLINIC01',
...
export const mockOrderIntakeService: IOrderIntakeService = {
  ...
  async resolveOrder(orderId: ID) {
    ...
    // ── Client resolution — Client.code IS the crosswalk key, no
    ...
    const clientMatch = clients.find(c => c.code.toLowerCase() === order.externalClientCode.toLowerCase());

    if (clientMatch) {
      order.clientId = clientMatch.id;
      order.clientWasAutoCreated = false;
    } else {
      const created = await mockClientService.findOrCreateByCode(
        order.externalClientCode,
        `Unrecognized client (order ${order.externalOrderNumber})`,
        `No Client.code match for "${order.externalClientCode}" on incoming order ${order.externalOrderNumber} — created pending admin review.`
      );
      if (created.ok) {
        order.clientId = created.data.id;
        order.clientWasAutoCreated = true;
        warnings.push(`No existing client matched code "${order.externalClientCode}" — created "${created.data.name}" as Unverified, pending admin review.`);
      }
    }

    // ── Per-specimen category resolution ────────────────────────────────
    const categoriesRes = await mockSpecimenCategoryService.getAll();
    ...
    const resolvedSpecimens = await Promise.all(order.specimens.map(async (spec) => {
      ...
      if (spec.externalSpecimenCode && order.clientId) {
        ...
        if (xwalkMatch && categories.some(c => c.id === xwalkMatch.specimenCategoryId)) {
          return { ...spec, specimenCategoryId: xwalkMatch.specimenCategoryId, categoryWasAutoCreated: false };
        }
      }

      // No crosswalk match (or no code at all) — fall back to
      // findOrCreateByName using whatever text is available, same
      // fail-open posture as everywhere else. A new crosswalk entry is
      // learned immediately so the same code resolves instantly next
      // time, even though the category itself still needs admin sign-off.
      const nameGuess = spec.externalSpecimenCode ?? spec.description;
      const created = await mockSpecimenCategoryService.findOrCreateByName(
        nameGuess,
        `No crosswalk match for specimen "${spec.description}"${spec.externalSpecimenCode ? ` (code "${spec.externalSpecimenCode}")` : ''} on order ${order.externalOrderNumber} from client code "${order.externalClientCode}" — created pending admin review.`
      );
      if (created.ok) {
        if (created.data.autoCreated && spec.externalSpecimenCode && order.clientId) {
          const newEntry: SpecimenCodeCrosswalkEntry = {
            ...
            specimenCategoryId: created.data.id,
        ...
        if (wasAutoCreated) {
          warnings.push(`No crosswalk match for specimen "${spec.description}" — created Specimen Category "${created.data.name}" as Unverified, pending admin review.`);
        }
        return { ...spec, specimenCategoryId: created.data.id, categoryWasAutoCreated: wasAutoCreated };
      }
      return spec;
    }));
