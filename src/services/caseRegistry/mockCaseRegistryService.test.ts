...
describe('mockCaseRegistryService', () => {
  ...
  it('allocates sequential numbers for a seeded organisation', async () => {
    const first = await mockCaseRegistryService.allocateNextCaseNumber('ORG-DVMC', 'America/Phoenix');
    const second = await mockCaseRegistryService.allocateNextCaseNumber('ORG-DVMC', 'America/Phoenix');
  ...
  it('never crashes for an organisation with no CaseMaskConfig — falls back to O{YEAR:2}-{SEQ:4}', async () => {
    const res = await mockCaseRegistryService.allocateNextCaseNumber('ORG-DOES-NOT-EXIST', 'America/Phoenix');
  ...
  it('two different unconfigured organisations get independent fallback sequences, not a shared one', async () => {
    const a1 = await mockCaseRegistryService.allocateNextCaseNumber('ORG-UNCONFIGURED-A', 'America/Phoenix');
    const b1 = await mockCaseRegistryService.allocateNextCaseNumber('ORG-UNCONFIGURED-B', 'America/Phoenix');
    const a2 = await mockCaseRegistryService.allocateNextCaseNumber('ORG-UNCONFIGURED-A', 'America/Phoenix');
  ...
  it('resolves a site prefix from sitePrefixMap when siteId is provided (a real, supported feature — just not used by MFT anymore, which now runs a unified sequence)', async () => {
    ...
    const res = await mockCaseRegistryService.allocateNextCaseNumber('ORG-TEST-MULTISITE', 'America/Phoenix', 'SITE-A');
    expect(res.ok).toBe(true);
    if (!res.ok) return;
  ...
  it('falls back to the organisation base prefix when siteId has no entry in sitePrefixMap', async () => {
    ...
    const res = await mockCaseRegistryService.allocateNextCaseNumber('ORG-TEST-MULTISITE', 'America/Phoenix', 'SITE-UNKNOWN');
    expect(res.ok).toBe(true);
    if (!res.ok) return;
  ...
  it('MFT runs a unified sequence — same prefix regardless of which real site is passed', async () => {
    const mri = await mockCaseRegistryService.allocateNextCaseNumber('ORG-MFT', 'America/Phoenix', 'SITE-MRI');
    const wyt = await mockCaseRegistryService.allocateNextCaseNumber('ORG-MFT', 'America/Phoenix', 'SITE-WYTH');
  ...
  it('falls back to the organisation base prefix when no siteId is given at all', async () => {
    const res = await mockCaseRegistryService.allocateNextCaseNumber('ORG-MFT', 'America/Phoenix');
    expect(res.ok).toBe(true);
    if (!res.ok) return;
  ...
  it('resets the sequence to 1 when lastResetYear is in the past and resetSequenceAnnually is true', async () => {
    ...
    const res = await mockCaseRegistryService.allocateNextCaseNumber('ORG-DVMC', 'America/Phoenix');
    expect(res.ok).toBe(true);
    if (!res.ok) return;
  ...
  it('previewNextCaseNumber shows the next number WITHOUT consuming it', async () => {
    const preview1 = await mockCaseRegistryService.previewNextCaseNumber('ORG-MPA', 'America/Phoenix');
    const preview2 = await mockCaseRegistryService.previewNextCaseNumber('ORG-MPA', 'America/Phoenix');
    ...
    const allocated = await mockCaseRegistryService.allocateNextCaseNumber('ORG-MPA', 'America/Phoenix');
  ...
  it('respects a custom sequenceDigits width', async () => {
    ...
    const res = await mockCaseRegistryService.allocateNextCaseNumber('ORG-HFHS', 'America/Phoenix');
    expect(res.ok).toBe(true);
    if (!res.ok) return;
    expect(res.data).toBe(`HFHS-${getFacilityDateParts(new Date(), 'America/Phoenix').year}-000001`);
  });
  ...
  it('a categoryOverride prefix takes precedence and draws from its own independent series', async () => {
    const surgical1 = await mockCaseRegistryService.allocateNextCaseNumber('ORG-DVMC', 'America/Phoenix', undefined, { prefix: 'S', numberSeries: 'SURGICAL' });
    const surgical2 = await mockCaseRegistryService.allocateNextCaseNumber('ORG-DVMC', 'America/Phoenix', undefined, { prefix: 'S', numberSeries: 'SURGICAL' });
  ...
  it('two different categoryOverride series at the same organisation never collide with each other or with the org default', async () => {
    const surgical = await mockCaseRegistryService.allocateNextCaseNumber('ORG-DVMC', 'America/Phoenix', undefined, { prefix: 'S', numberSeries: 'SURGICAL' });
    const cytology = await mockCaseRegistryService.allocateNextCaseNumber('ORG-DVMC', 'America/Phoenix', undefined, { prefix: 'NG', numberSeries: 'CYTOLOGY_NONGYN' });
    const orgDefault = await mockCaseRegistryService.allocateNextCaseNumber('ORG-DVMC', 'America/Phoenix');
  ...
  it('a categoryOverride with no numberSeries shares the organisation default series (e.g. Frozen Section sharing Surgical\'s series in real usage)', async () => {
    const withPrefixNoSeries = await mockCaseRegistryService.allocateNextCaseNumber('ORG-DVMC', 'America/Phoenix', undefined, { prefix: 'DVMC' });
    const orgDefault = await mockCaseRegistryService.allocateNextCaseNumber('ORG-DVMC', 'America/Phoenix');
  ...
  it('categoryOverride works even for an organisation with no CaseMaskConfig at all — still gets the category prefix, not the generic O fallback', async () => {
    const res = await mockCaseRegistryService.allocateNextCaseNumber('ORG-TEST-UNCONFIGURED', 'America/Phoenix', undefined, { prefix: 'S', numberSeries: 'SURGICAL' });
    expect(res.ok).toBe(true);
    if (!res.ok) return;
  ...
  it('previewNextCaseNumber respects categoryOverride and does not consume its series counter', async () => {
    const preview1 = await mockCaseRegistryService.previewNextCaseNumber('ORG-DVMC', 'America/Phoenix', undefined, { prefix: 'CS', numberSeries: 'CONSULTATION' });
    const preview2 = await mockCaseRegistryService.previewNextCaseNumber('ORG-DVMC', 'America/Phoenix', undefined, { prefix: 'CS', numberSeries: 'CONSULTATION' });
    expect(preview1.ok && preview2.ok).toBe(true);
    if (!preview1.ok || !preview2.ok) return;
    expect(preview1.data).toBe(preview2.data);
