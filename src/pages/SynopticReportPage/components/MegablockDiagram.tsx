// src/pages/SynopticReportPage/components/MegablockDiagram.tsx
// ─────────────────────────────────────────────────────────────────────────────
...
// megablock sizes (2-12 positions — a full prostate mapping biopsy is the
...
export interface MegablockPosition {
  position: number;
...
interface MegablockDiagramProps {
  cassetteLabel: string;
  positions: MegablockPosition[];
...
// square regardless of how many positions it holds (a 3-core megablock
...
const MegablockDiagram: React.FC<MegablockDiagramProps> = ({ cassetteLabel, positions, onOpenBlockEditor }) => {
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginLeft: 40, marginBottom: 12 }}>
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ flexShrink: 0 }}>
        {/* Cassette outline — same amber/brown language as BlockIcon, so a
            megablock still visually reads as "a block" at a glance. */}
        <rect
...
export default MegablockDiagram;
